...
<head>
    ...
    <title>QR Pinhal - Listar Turismo</title>
    ...
    <main> <?php
            ...
            if (mysqli_num_rows($consulta) == 0) {
                ...
                while ($dados = mysqli_fetch_array($consulta)) {
                    ...
                    echo "
            <article onclick=\"window.location='detalhes_adm.php?id={$dados['id_turismo']}'\">   
            <h5>{$dados['id_turismo']}</h5>
            <h2>{$dados['titulo']}</h2>
            <h4>{$dados['informacoes']}</h4>
            <h3>{$dados['nome']}</h3>
            <img src='img/dados_imagem/$dados[3]' style='width:15%;border-radius:0px 0px 8px 8px'>
        </article>
        ";
                }
            }
